...
//merging
    ...
    for(int i=0; i<newrow; i++)
    {
        ...
        if(flag)//if no duplicate value found
        {
            for(int j=0; j<COL; j++)
            {
            ...
            else
            {
                ...
                if(flag2)
                {
                    ...
                    for(int utl=0; utl<=uq_idIndex; utl++)
                    {
                        ...
                        if(unique_idlist[utl].compare(scope_+temp)==0)//variable replace
                        {
                            flag=true;
...
void errorFinder(regex rgx,string text)
{
    ...
    for( sregex_iterator it(text.begin(), text.end(), rgx), it_end; it != it_end; ++it )
    {
        ...
        if (temp.find("flc") != string::npos)lineno = temp[0];
...
void work4()
{
    ...
    else
    {
        while (!file.eof())
        {
            i++;
            getline(file, temp);
            text = text + to_string(i)+"flc"+" "+temp + "\n";
        }
